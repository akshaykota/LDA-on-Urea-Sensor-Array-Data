...
trainData = csvread('data1pcConc.csv'); % Load Training Data
testData = csvread('testdata1pcConc.csv'); % Load Testing Data
...
% Projecting the test data onto eigenvector corresponding 
% to highest eigenvalue and classifying it
xin = testData(:,1:end-1)'; % Get the test data in matrix
transXin = eigvec(:,3)'*xin; % Transform the test data by multiplying with eigenvectors
transMeanVec = eigvec(:,3)'*classMeanVec'; % Transform mean of each class by multiplying with eigenvectors
...
transXtrain = eigvec(:,3)'*trainData(:,1:3)';
...
title('Scatter Plot of Initial Data');xlabel('Feature 1 (D.1) -->');
ylabel('Feature 2 (E.1) -->');
